...
def get_hotels_bestdeal(city_id, search_info, count, start_price, stop_price, start_dist, stop_dist, bestdeal_list, page_num='1'):
    ...
    try:
        ...
        while True:
            ...
            for elem in data["data"]["body"]["searchResults"]["results"]:
                ...
                try:
                    hotel_price = data["data"]["body"]["searchResults"]["results"][number]["ratePlan"]["price"][
                            "current"]
                    hotel_price = str(hotel_price).replace(',', '.')
                except Exception:
                    hotel_price = 'Цена отсутствует'
